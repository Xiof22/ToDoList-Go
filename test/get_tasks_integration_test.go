...
func TestGetTasks(t *testing.T) {
	...
	t.Run("Have task", func(t *testing.T) {
		...
		assert.Equal(t, wantResponse.Count, gotResponse.Count)
		assert.Equal(t, wantResponse.Tasks[0].Title, gotResponse.Tasks[0].Title)
		assert.Equal(t, wantResponse.Tasks[0].Deadline, gotResponse.Tasks[0].Deadline)
	})
}
